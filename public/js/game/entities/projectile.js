ig.module('game.entities.projectile').requires(
	'impact.entity'
)
.defines(function(){
EntityProjectile = ig.Entity.extend({
		//Superficial values
		swingSounds: [new ig.Sound('/audio/slash1.*'), new ig.Sound('/audio/slash2.*'), new ig.Sound('/audio/slash3.*')],
		hitSound: new ig.Sound('/audio/slashHit1.*'),
		...
		init: function(x, y, settings) {
			...
			this.hangTime = new ig.Timer(.18);
		},
		
		update: function() {
			if (this.hangTime.delta() > 0) this.kill();
			else {
		...
		check: function( other ) {
			if (other.hitCooldown) {
				if (other.hitCooldown.delta() > 0) {
					other.hitCooldown.set(-this.hangTime.delta());
